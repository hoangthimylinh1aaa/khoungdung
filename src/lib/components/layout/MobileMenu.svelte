...
<!-- Drawer -->
<div
	class="fixed top-0 right-0 z-50 h-full w-72 transition-transform duration-300 ease-out md:hidden"
	...
	<div class="flex h-full flex-col p-6 pt-24">
		...
		<div class="mt-auto flex flex-col gap-3">
			<button
				...
				class="glass glass-hover flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-medium text-gray-300"
			>
				<span>{locale === 'vi' ? '🇻🇳 Tiếng Việt' : '🇬🇧 English'}</span>
				<span class="ml-auto text-xs text-gray-500">→ {locale === 'vi' ? 'EN' : 'VI'}</span>
			</button>
			<a href="/login" on:click={() => (open = false)} class="btn-outline w-full justify-center">
				{nav.login}
			</a>
			<a href="/register" on:click={() => (open = false)} class="btn-primary w-full justify-center">
				{nav.register}
			</a>
		</div>
	</div>
