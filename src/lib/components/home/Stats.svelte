...
>
	...
	<div class="container-custom">
		<AnimatedText class="mb-12 text-center">
			<h2 id="stats-heading" class="font-display text-3xl font-bold text-white sm:text-4xl">
		...
		<div class="grid grid-cols-2 gap-4 sm:gap-6 lg:grid-cols-4">
			{#each statKeys as key, i (`${key}-stats`)}
				...
				<AnimatedText delay={i * 100} direction="up">
					<div class="card-glow group p-6 text-center sm:p-8">
						<div
							class="bg-primary/10 border-primary/20 group-hover:bg-primary/20 mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-2xl border transition-colors"
						>
							<svg
						...
						<div class="font-display text-gradient mb-1 text-3xl font-black sm:text-4xl">
							{stat.value}
						</div>
